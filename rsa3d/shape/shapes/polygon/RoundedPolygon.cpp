...
bool RoundedPolygon::overlapComplexCheck(Vector<2> &position, double angle, Vector<2> &polposition, double polangle) const{
	...
	double circumsphereRadius2 = std::pow(Shape::getCircumsphereRadius() + RoundedPolygon::radius, 2);
...
double RoundedPolygon::getArea() {
    ...
    // now we add rectangles for each segment
    size_t i0, i1;
    ...
    area += M_PI*RoundedPolygon::radius*RoundedPolygon::radius;
    return area;
}
...
std::string RoundedPolygon::toPovray() const{
	...
	for (size_t i=0; i < Polygon::segments.size(); i++) {
		...
		Vector<2> vSegment, vNormal, vBegin, vEnd;
