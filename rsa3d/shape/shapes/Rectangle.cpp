...
double Rectangle::getXOnCircle(double cx, double cy, double r,
		double angle) const {
	return cx + cos(angle) * r;
}

double Rectangle::getYOnCircle(double cx, double cy, double r,
		double angle) const {
...
// never used
bool Rectangle::pointInside(BoundaryConditions<2> *bc,
	const Vector<2> &da) const {
// apply bc
Vector<2> position = this->getPosition();
Vector<2> newDa = da + bc->getTranslation(position, da);

// rotate point relatively to rectangle center
Vector<2> pointRotated = rotatePoint(newDa, getAntiRotationMatrix(),
		Vector<2>(position));
...
bool Rectangle::pointInside(BoundaryConditions<2> *bc, const Vector<2> &da,
	double angleFrom, double angleTo) const {
...
bool Rectangle::pointInsideInternal(BoundaryConditions<2> *bc,
	const Vector<2> &da, double angleFrom, double angleTo) const {
double pi2 = M_PI / 2;
int countFrom = (int) floor(angleFrom / pi2);
...
bool Rectangle::checkTangents(double x, double y, double cx, double cy,
	double r, double angle1, double angle2) const {
// calculate tangents
...
bool Rectangle::isInsideExcludingRectangle(double angleFrom, double angleTo,
...
// it's math abs
if (std::fabs(x - position[0]) <= newHalfA
		&& std::fabs(y - position[1]) <= newHalfB) {
	return 1;
}
return 0;
}

int Rectangle::isInsideCircle(const Vector<2, double> &point, double cx,
	double cy, double r) const {
...
int Rectangle::isInsideRect(const Vector<2, double> &point, double cx,
	double cy, double aLength, double bLength) const {
if (std::fabs(cx - point[0]) <= aLength / 2
		&& std::fabs(cy - point[1]) <= bLength / 2) {
	return 1;
}
return 0;
}

Vector<2> Rectangle::rotatePoint(const Vector<2> &point,
	const Matrix<2, 2> &rotation) const {
return rotation * point;
}

Vector<2> Rectangle::rotatePoint(const Vector<2> &point,
	const Matrix<2, 2> &rotation, const Vector<2> &center) const {
...
bool Rectangle::isIntersection(double p0_x, double p0_y, double p1_x,
	double p1_y, double p2_x, double p2_y, double p3_x, double p3_y) const {
...
std::vector<double> Rectangle::calculateOrder(
	const OrderCalculable *other) const {
...
double angle = (this->getOrientation())[0]
		- (otherRectangle.getOrientation())[0];
...
out << std::fixed;
out << "GeometricTransformation[Rectangle[{" << -halfA << "," << -halfB
		<< "}, {" << halfA << "," << halfB << "}]," << std::endl;
...
out
		<< "  polygon{5, <0.5, 0.5, 0.0002>, <-0.5, 0.5, 0.0002>, <-0.5, -0.5, 0.0002>, <0.5, -0.5, 0.0002>, <0.5, 0.5, 0.0002>"
		<< std::endl;
;
...
out << "0.0>" << std::endl << "	texture { pigment { color Red } }" << std::endl
		<< "}" << std::endl;
