...
#include "shapes/DiscreteOrientationsShape2_1.h"
		...
		#include "shapes/GenericXCShape.h"
		#include "shapes/RoundedCone.h"
    #endif
#endif
    ...
    //------------------------------
        ...
        // 3D shapes with no angular dimension
            ...
            if (sClass == "Cuboid") {
                ...
                return;
            } else if (sClass == "GenericXCShape") {
                GenericXCShape::initClass(attr);
                return;
            } else if (sClass == "RoundedCone") {
                RoundedCone::initClass(attr);
                return;
...
VoxelList *ShapeFactory::createVoxelList(const std::string &sClass, const std::string &attr, unsigned short surfaceDimension, double spatialSize,
                                         double voxelSpatialSize, const RSAOrientation &angularSize,
                                         const RSAOrientation &requestedAngularVoxelSize)
{
    #if RSA_ANGULAR_DIMENSION == 0
