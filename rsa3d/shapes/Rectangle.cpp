...
double Rectangle::getVoxelAngularSize() {
    return M_PI / 2;
}

void Rectangle::translate(double *v) {
    Shape::translate(v);
    for (int i = 0; i < 5; i++) {
        xs[i] += v[0];
        ys[i] += v[1];
...
int Rectangle::pointInsideInternal(BoundaryConditions *bc, double *da, double angleFrom, double angleTo) {
    double pi2 = M_PI / 2;
    int countFrom = (int) floor(angleFrom / pi2);
...
bool Rectangle::isInsideExcludingRectangle(double angleFrom, double angleTo, double x, double y) const {
    ...
    for (int i = 0; i < 4; i++) {
        newSizeFrom[0] = std::max(newSizeFrom[0], rec.xs[i] - rec.position[0]);
        newSizeFrom[1] = std::max(newSizeFrom[1], rec.ys[i] - rec.position[1]);
    }
    rec.setAngle(angleTo);
    for (int i = 0; i < 4; i++) {
        newSizeTo[0] = std::max(newSizeTo[0], rec.xs[i] - rec.position[0]);
        newSizeTo[1] = std::max(newSizeTo[1], rec.ys[i] - rec.position[1]);
    }
...
int Rectangle::isInsideRect(const Vector<2, double> &point, double cx, double cy, double aLength, double bLength) {
...
double Rectangle::isLeft(double p0x, double p0y, double p1x, double p1y, double p2x, double p2y) {
    return ((p1x - p0x) * (p2y - p0y) - (p2x - p0x) * (p1y - p0y));
}

int Rectangle::wn_PnPoly(double x, double y, double *xs, double *ys, int n) {
...
int Rectangle::overlap(BoundaryConditions *bc, Shape<2, 1> *s) {
    ...
    if (wn_PnPoly(rectangle.position[0], rectangle.position[1], xs, ys, 4) != 0) {
        return 1;
    }
