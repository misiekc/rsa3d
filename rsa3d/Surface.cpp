...
Surface::Surface(int dim, double s, double ndx, double vdx, std::unique_ptr<RSABoundaryConditions> bc) {
	...
	if (s <= 2*ndx)
		std::cout << "[Surface::Surface] Packing linear size (" << s << ") is <= 2 neighbour list cell size (" << ndx << ") - no neighbour grid support, boundary conditions may not be applied" << std::endl;
