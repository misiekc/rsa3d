...
unsigned short VariableSizeVoxelList::splitVoxels(double minDx, size_t maxVoxels, NeighbourGrid<const RSAShape> *nl, RSABoundaryConditions *bc, bool verbose){
	if (this->disabled)
	...
	for(size_t i=0; i<this->length; i++){
		...
		if (verbose && i%10000 == 0){ std::cout << "." << std::flush; }
	}
